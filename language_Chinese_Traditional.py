# coding=utf-8
# Copyright ©2025 ZHJ.

FOR = ['FilesChecker', (3, 1, 3), 'Windows']
LANGUAGE = [['Chinese (Traditional)_Taiwan', 'Chinese (Traditional)_Hong Kong SAR', 'Chinese (Traditional)_Macao SAR'], '繁體中文 (中國香港特別行政區、中國澳門特別行政區、中國臺灣)', 'zhtr']
def s1():
    return '文件校驗器'
def s2():
    return '開始校驗(&K)'
def s3():
    return '添加文件(&F)…'
def s4():
    return '添加校驗和(&H)…'
def s5():
    return '導入任務清單(&O)…'
def s6():
    return '保存任務清單(&S)…'
def s7():
    return '匯出校驗結果(&E)…'
def s8():
    return '清空列表(&C)'
def s9():
    return '設置(&S)…'
def s10():
    return '退出(&X)'
def s11():
    return '任務(&T)'
def s12():
    return '關於(&A)…'
def s13():
    return '幫助(&H)'
def s14():
    return '添加文件(&F)…'
def s15():
    return '添加校驗和(&H)…'
def s16():
    return '開始校驗(&K)'
def s17():
    return '序號'
def s18():
    return '組號'
def s19():
    return '文件'
def s20():
    return '校驗和'
def s21():
    return '自動'
def s22():
    return '添加文件'
def s23():
    return '所有檔案 (*.*)|*.*'
def s24():
    return '設定'
def s25():
    return '完成後操作'
def s26():
    return '無'
def s27():
    return '關機'
def s28():
    return '重啟'
def s29():
    return '自訂…'
def s30():
    return '自動保存校驗結果'
def s31():
    return '自動保存校驗結果'
def s32():
    return '匯出校驗結果'
def s33():
    return 'Microsoft Excel 逗號分隔值文件 (*.csv)|*.csv|文字文件 (*.txt)|*.txt|所有檔案 (*.*)|*.*'
def s34():
    return '常規'
def s35():
    return '語言設置'
def s36(list):
    if list:
        return '雜湊數值型別：' + ' 或 '.join(map(str, list))
    else:
        return '雜湊數值型別：未知'
def s37():
    return '應用(&A)'
def s38():
    return '刪除(&D)'
def s39():
    return '需要重新開機軟體以更改語言。你確定要繼續嗎？'
def s40():
    return '語言'
def s41():
    pass
def s42():
    return '添加校驗和'
def s43():
    pass
def s44():
    return '關於'
def s45(ver):
    return '文件校驗器 v' + '.'.join(map(str, ver))
def s46():
    return '未安裝該語言包。'
def s47(pyver, wxver):
    return 'Python 版本：' + pyver + '    wxPython 版本：' + wxver
def s48():
    return '文字文件 (*.txt)|*.txt|所有檔案 (*.*)|*.*'
def s49():
    return '導入任務清單'
def s50():
    return '打開文本文檔'
def s51():
    return '請選擇文本文檔使用的編碼。'
def s52():
    return '預覽：'
def s53():
    return '確定'
def s54():
    return '取消'
def s55():
    return '無法讀取文件：'
def s56():
    return '無法讀取任務清單：'
def s57():
    return '確定(&O)'
def s58():
    return '任務清單錯誤：不支援的演算法。'
def s59():
    return '任務清單錯誤：未定義資料類型。'
def s60():
    return '保存任務清單'
def s61():
    return '任務清單中不含任何可添加資料。'
def s62():
    return '警告'
def s63():
    return '是(&Y)'
def s64():
    return '否(&N)'
def s65():
    return '正在校驗，確認退出？'
def s66():
    return '已用時間：'
def s67():
    return '已處理：'
def s68():
    return '進度：'
def s69():
    return '總進度：'
def s70():
    return '校驗和計算'
def s71():
    return '正在處理：'
def s72():
    return '大小：'
def s73():
    return '位元組'
def s74():
    return '時間：'
def s75(err):
    return '發生錯誤，以下是錯誤資訊：\n' + err
def s76():
    return '顯示詳細資訊(&S)'
def s77():
    return '重新選擇檔(&C)…'
def s78():
    return '編輯校驗和(&E)…'
def s79():
    return '複製檔地址(&F)'
def s80():
    return '複製校驗和(&H)'
def s81():
    return '錯誤'
def s82():
    return '正在計算校驗和…'
def s83():
    return '正在校驗…'
def s84():
    return '正在保存校驗結果：'
def s85():
    return '正在運行命令：'
def s86():
    return '正在保存任務清單：'
def s87():
    return '重新選擇檔'
def s88():
    return '編輯校驗和'
def s89():
    return '已複製到剪貼板。'
def s90():
    return '運行命令：'
def s91():
    return '設置項"自動保存校驗結果"無效。'
def s92():
    return '詳細資訊'
def s93():
    return '文件：'
def s94():
    return '校驗和：'
def s95():
    return '計算耗時：'
def s96():
    return '錯誤資訊：'
def s97():
    return '計算校驗和並校驗。'
def s98():
    return '添加一個或多個檔。'
def s99():
    return '添加校驗和。'
def s100():
    return '從檔載入任務清單。'
def s101():
    return '將任務清單保存到檔。'
def s102():
    return '將校驗結果匯出到檔。'
def s103():
    return '更改軟體的設置。'
def s104():
    return '退出本軟體。'
def s105():
    return '顯示軟體資訊、版本號和版權。'
def s106():
    return '任務清單'
def s107():
    return '保存任務清單時使用編碼：'
def s108():
    return '校驗結果'
def s109():
    return '匯出校驗結果時使用編碼：'
def s110():
    return '恢復默認設置(&R)'
def s111():
    return '檔案編碼'
def s112():
    return '用戶取消了操作。'
def s113():
    return '暫停(&P)'
def s114():
    return '繼續(&C)'
def s115():
    return '你確定要取消嗎？'
def s116():
    return '軟體更新(&U)…'
def s117():
    return '軟體更新'
def s118(ver):
    return '當前版本：' + '.'.join(map(str, ver))
def s119(version):
    if version:
        return '最新版本：' + '.'.join(map(str, version))
    else:
        return '最新版本：'
def s120():
    return '正在檢查更新…'
def s121():
    return '下载进度：'
def s122():
    return '檢查(&C)'
def s123():
    return '連接中…'
def s124():
    return '更新(&U)'
def s125():
    return '補丁包大小：'
def s126():
    return '完整包大小：'
def s127():
    return '無法獲取更新，請稍後再試。'
def s128():
    return '剩餘'
def s129():
    return '工作目錄(&D)…'
def s130():
    return ['模式 1', '模式 2']
def s131():
    return '添加項目(&A)…'
def s132():
    return '添加一個項目。'
def s133():
    return '添加項目(&A)…'
def s134():
    return '預期校驗和'
def s135():
    return '校驗結果'
def s136():
    return '設定工作目錄'
def s137():
    return '切換模式將清空清單，是否繼續？'
def s138():
    return '預期校驗和：'
def s139():
    return '有軟體更新可用。'
def s140():
    return '添加項目'
def s141():
    return '文件：'
def s142():
    return '預期校驗和：'
def s143():
    return '檔案無效'
def s144():
    return '重新選擇檔案並重試。'
def s145():
    return '粘貼或從檔案加載任務清單。'
def s146():
    return '加載檔案(&F)…'
def s147():
    return '所有支持的檔案 (*.md5; *.sha; *.sha1; *.sha256; *.txt)|*.md5; *.sha; *.sha1; *.sha256; *.txt|文字文件 (*.txt)|*.txt|所有檔案 (*.*)|*.*'
def s148():
    return '啟動軟件時'
def s149():
    return '默認校驗模式：'
def s150():
    return '另一個實例正在運行。'
def s151():
    return '正在校驗：'
def s152():
    return '更新檔案校驗錯誤，請檢查網絡連接。'
